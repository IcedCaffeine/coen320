...
int SecondaryRadar::start() {
	time(&startTime);
	if (pthread_create(&secondaryRadarThread, &attr, &SecondaryRadar::startSecondaryRadar, (void *)this) != EOK) {
		secondaryRadarThread = 0;
	}
	return 0;
}

std::vector<std::string> SecondaryRadar::getWaitingFileNames() const {
	return waitingFileNames;
}

void SecondaryRadar::setWaitingFileNames(std::vector<std::string> waitingFileNames) {
	this->waitingFileNames = waitingFileNames;
}

int SecondaryRadar::stop() {
	pthread_join(secondaryRadarThread, NULL);

	return 0;
}

int SecondaryRadar::getCurrentPeriod() const {
	return currentPeriod;
}

void SecondaryRadar::setCurrentPeriod(int currentPeriod) {
	this->currentPeriod = currentPeriod;
}

std::vector<std::string> SecondaryRadar::getFileNames() const {
	return fileNames;
}

void SecondaryRadar::setFileNames(std::vector<std::string> fileNames) {
	this->fileNames = fileNames;
}

std::vector<std::string> SecondaryRadar::getFlyingFileNames() const {
	return flyingFileNames;
}

void SecondaryRadar::setFlyingFileNames(std::vector<std::string> flyingFileNames) {
	this->flyingFileNames = flyingFileNames;
}

int SecondaryRadar::getNumberOfPlanes() const {
	return numberOfPlanes;
}

void SecondaryRadar::setNumberOfPlanes(int numberOfPlanes) {
	this->numberOfPlanes = numberOfPlanes;
}

Timer* SecondaryRadar::getTimer() const {
	return timer;
}

void SecondaryRadar::setTimer(Timer *timer) {
	this->timer = timer;
}
...
void SecondaryRadar::updatePeriod() {
	...
	if (newPeriod != this->getCurrentPeriod()) {
		...
		this->getTimer()->setTimer(this->getCurrentPeriod(), this->getCurrentPeriod());
	}
}

bool SecondaryRadar::readFlyingPlanes() {
	bool write = false;
	std::string fileDataBuffer = "";
	...
	for (int i = 0; i < SIZE_SHM_SSR; i++) {
		...
		if (readChar == ';') {
			...
			if (!inFile) {
				...
				auto itName = this->getWaitingFileNames().begin();
				auto itPtr = waitingPtrs.begin();
				while (itName != this->getWaitingFileNames().end() && itPtr != waitingPtrs.end()) {
					if ((*itName) == fileDataBuffer) {
						this->getFlyingFileNames().push_back((*itName));
						planePtrs.push_back((*itPtr));
						itName = this->getWaitingFileNames().erase(itName);
						itPtr = waitingPtrs.erase(itPtr);
						break;
					}
					++itName;
					++itPtr;
